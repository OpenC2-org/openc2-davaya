...
def _write_field(sheet, row, col, f, fmt):
    sheet.write(row, col, f[0], fmt[2])
    sheet.write(row, col + 1, f[1], fmt[2])
    sheet.write(row, col + 2, f[2], fmt[2])
    # process options f[3]
    if len(f) > 4:
        sheet.write(row, col + 3, f[4], fmt[2])
    return 1
...
def table_dump(jaen, fname, source=""):
    ...
    tprop = wkbook.add_format({"text_wrap": True, "border": 1})
    ...
    vrow, vcol = 3, 1
    trow, tcol = 3, 1
    _set_col_widths(sheet_vocab, 0, [12, 4, 20, 50])
    _set_col_widths(sheet_types, 0, [12, 4, 20, 20, 50])
    fmt = [tdname, tdval, tdfield]
    for n, t in enumerate(jaen["types"]):   # 0: type name, 1: base type, 2:opts, 3: desc, 4:fields
        if t[1] == "Enumerated":
            vrow += _write_type(sheet_vocab, vrow, 0, 3, "Vocabulary:", t[0], fmt)
            if t[2]:
                vrow += _write_type(sheet_vocab, vrow, 0, 3, "Options:", t[2], fmt)
            if t[3]:
                vrow += _write_type(sheet_vocab, vrow, 0, 3, "", t[3], fmt)
            _write_row(sheet_vocab, vrow + 1, vcol, ["Id", "Value", "Description"], thead)
            vrow += 2
            for nf, f in enumerate(t[4]):
                vrow += _write_field(sheet_vocab, vrow, vcol, f, fmt)
            vrow += 1
        else:
            trow += _write_type(sheet_types, trow, 0, 4, "Type Name:", t[0], fmt)
            trow += _write_type(sheet_types, trow, 0, 4, "Type:", t[1], fmt)
            if t[2]:
                trow += _write_type(sheet_types, trow, 0, 4, "Options:", json.dumps(t[2]), fmt)
            if t[3]:
                trow += _write_type(sheet_types, trow, 0, 4, "", t[3], fmt)
            ...
            if len(t) > 4:      # field table without header indicates something wrong
                for nf, f in enumerate(t[4]):
                    trow += _write_field(sheet_types, trow, tcol, f, fmt)
            trow += 1
